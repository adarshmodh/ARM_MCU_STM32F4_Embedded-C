	...
	if(TIM_GetITStatus(TIM4, TIM_IT_CC2) == SET) 
  {
    ...
    else if(CaptureNumber == 1)
    {
      ...
      TIM4Freq = (uint32_t) SystemCoreClock / Capture/100;
			...
			//LCD display
			clearLCD(); WriteLCDLine1("RPM              ");displayvalue(RPM);//delay(1000000);
...
void ADC_IRQHandler(void)
{
    ...
    ADC_ClearITPendingBit(ADC3, ADC_IT_EOC );
		...
		if(flag==30)
		{
			clearLCD();WriteLCDLine1("Temperature       ");displayvalue(temp);delay(70000000);
			flag=0;
		}	
		else
		flag++;	
...
void TIM3_config(void)
{
  ...
  TIM_TimeBaseStructure.TIM_Prescaler = (uint16_t) ((SystemCoreClock)/2 / (1000)) - 1;// 500ms delay
